...
/**
 * Verify that we can create a valid deck, draw all its cards, runGameLoop them,
 * and everything is functional
 */
void testCards() {
  std::vector<Card *> cards = {new BombCard(), new AirliftCard(),
							   new NegotiateCard(), new BlockadeCard()};
  ...
  Utils::assertCondition(playerOne->hand->cards.size() == cards.size(),
						 "player drew all the cards in the deck");

  while (!playerOne->hand->cards.empty()) {
	auto cardName = playerOne->hand->cards[0]->name;
	cout << "Playing " << cardName << endl;
	// We use debugPlay instead of play so that we don't have to put a bunch of
	// user input
	playerOne->hand->debugPlay(cardName);
  }

  Utils::assertCondition(playerOne->hand->cards.empty(),
						 "all the cards were removed from the player's hand");
  Utils::assertCondition(ge->deck->getCardsSize() == cards.size(),
						 "all the cards were put in the deck");
