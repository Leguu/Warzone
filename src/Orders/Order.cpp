...
void DeployOrder::validate() {
    if (issuer == nullptr) {
        throw InvalidOrderException(
                "One of the areas is now a blockaded neutral territory. Can no longer continue action");
    }
...
void AdvanceOrder::validate() {
    ...
    } else if (source->getOwner() && target->getOwner() && source->getOwner() != target->getOwner() &&
...
void AdvanceOrder::execute() {
    ...
    if (armies == target->getArmies()) {
        ...
        if(target->getOwner() != nullptr){
            target->getOwner()->removeFromOwnedTerritories(target->getName());
        }
        target->setOwner(this->issuer);
        source->getOwner()->removeFromOwnedTerritories(target->getName());
        target->setArmies(abs(target->getArmies()));
        issuer->ownedTerritories.push_back(target);
...
void BombOrder::validate() {
    if (issuer && target->getOwner() && (target->getOwner()->name == issuer->name)) {
...
void NegotiateOrder::validate() {
    if (this->target == nullptr || this->issuer == nullptr) {
        throw InvalidOrderException(
                "One of the areas is now a blockaded neutral territory. Can no longer continue action");
    }
