...
void testCommandProcessor() {
  ...
  ge->transition(GameEngine::MAP_LOADED);
  cout << "You are currently in state "
	   << GameEngine::gameStates[ge->getState()] << endl;

  while (true) {
	auto input1 =
		Utils::getInputString("Where do you want to receive commands?\n"
							  "Enter either file or console. Type leave to go "
	...
	if (input1 == "console") {
	  ...
	  string fileName =
		  Utils::getInputString("Enter the name of the text file (with the "
	  ...
	  while (!fcpa->flr->ifile.eof()) {
		...
		if (input2 == "Y") {
		  ...
		  cout << "invalid command. Y or N only";
	  }
